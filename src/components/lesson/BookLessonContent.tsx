...
function getDistractorDefinitions(details: BookWordDetail[], excludeTerm: string): string[] {
  return details
    .filter((detail) => detail.term !== excludeTerm)
...
function createCorrectSentence(detail: BookWordDetail): string {
  ...
  if (detail.partOfSpeech === 'verb') {
    return `They ${detail.term} when the situation matches the definition.`
  }

  if (detail.partOfSpeech === 'adjective') {
    return `The situation feels ${detail.term} in this example.`
  }

  if (detail.partOfSpeech === 'adverb') {
    return `The team worked ${detail.term} to finish the task.`
  }
...
function createIncorrectSentence(detail: BookWordDetail): string {
  if (detail.partOfSpeech === 'verb') {
    return `The notebook ${detail.term} blue and square every morning.`
  }

  if (detail.partOfSpeech === 'adjective') {
    return `We can ${detail.term} the homework with a pencil.`
  }

  if (detail.partOfSpeech === 'adverb') {
    return `The ${detail.term} sat on the desk beside the lamp.`
  }
  ...
  return (
    <div className="space-y-8">
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <h2 className="text-lg font-bold text-gray-900">Unit Overview</h2>
        <p className="mt-2 text-sm text-gray-600">
          This lesson now follows the same exercise flow as the book: target words,
          definition checks, sentence sense checks, story reading, and answer questions.
        </p>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <h2 className="text-lg font-bold text-gray-900">Target Words</h2>
        <p className="text-sm text-gray-400">20 words from {lesson.title}</p>
            ...
            >
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="font-semibold text-gray-900">{detail.term}</span>
                  {detail.ipa ? <span className="text-sm text-primary-600">{detail.ipa}</span> : null}
                  <span className="rounded-full bg-white px-2 py-0.5 text-xs text-gray-500">
                    {detail.partOfSpeech}
                  </span>
                </div>
                <p className="mt-1 text-sm text-gray-700">{detail.definition}</p>
                {detail.definitionVi ? (
                  <p className="mt-1 text-sm text-emerald-700">{detail.definitionVi}</p>
                ) : null}
                {detail.example ? (
                  <p className="mt-1 text-sm italic text-gray-500">"{detail.example}"</p>
                ) : null}
              </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <h2 className="text-lg font-bold text-gray-900">{exerciseGroups[3].title}</h2>
        <div className="mt-4 space-y-6">
